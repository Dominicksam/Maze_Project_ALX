...
void draw(SDL_Instance instance, char **map, double_axis play, double_axis dir,
	  double_axis plane)
{
 ...
 **/
void draw_walls(char **map, double_axis play, SDL_Instance instance, double_axis dir,
		double_axis plane)
{
	double_axis ray_pos, ray_dir, dist_side, dist_del;
 ...
 **/
void choose_color(SDL_Instance instance, char **map, int_axis coord, int hit_side)
{
		...
		case '1':
			/* Brown walls */
			if (hit_side == 0)
				SDL_SetRenderDrawColor(instance.renderer, 0xA5, 0x2A, 0X2A, 0xFF);
			else
		...
		case '3':
			/* Purple Walls */
			if (hit_side == 0)
				SDL_SetRenderDrawColor(instance.renderer, 0x80, 0, 0x80, 0xFF);
			else
		...
		case '4':
			/* Orange Walls */
			if (hit_side == 0)
				SDL_SetRenderDrawColor(instance.renderer, 0xFF, 0xA5, 0, 0xFF);
			else
