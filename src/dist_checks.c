...
void check_ray_dir(int_axis *step, double_axis *dist_side, double_axis ray_pos,
		   int_axis coord, double_axis dist_del, double_axis ray_dir)
{
	if (ray_dir.x < 0)
	{
		step->x = -1;
 ...
 **/
double get_wall_dist(char **map, double_axis *dist_side, int_axis *coord,
		      int_axis *step, double_axis *dist_del, int *hit_side,
		      double_axis *ray_dir, double_axis *ray_pos)
{
	double wall_dist;
	int hit_wall = 0;
